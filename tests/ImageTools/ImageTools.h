...
rif_image LoadBinImage(const std::string &path, int width, int height, int cnum, rif_context context,
                       rif_component_type type = RIF_COMPONENT_TYPE_FLOAT32) {
...
rif_image LoadImage(const std::filesystem::path &file_path, rif_context context) {
    rif_image img = nullptr;
    ...
    } else if (ext == "png" || ext == "bmp" || ext == "tga") // unsigned int8  type output file
    {
        ...
        switch (type) {
            ...
            case RIF_COMPONENT_TYPE_FLOAT32:
                ...
                break;
            case RIF_COMPONENT_TYPE_FLOAT16:
                CopyAndConvert<rif_uchar, half_float::half>(rawData, (half_float::half *) data, arraySize, 255U,
                                                            true);
                break;
            default:
                return RIF_ERROR_UNSUPPORTED;
